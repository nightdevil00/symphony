...
play_local_music() {
  ...
  choice=$(printf "%s\n" "${filenames[@]}" | rofi -i -dmenu -config ~/.config/rofi/custom-rofi/config-rofi-Beats.rasi -p "Local Music")
...
play_online_music() {
  choice=$(printf "%s\n" "${!online_music[@]}" | rofi -i -dmenu -config ~/.config/rofi/custom-rofi/config-rofi-theme.rasi -p "Online Music")
...
pkill mpv && notify-send -u low -i "$iDIR/music.png" "Music stopped" || {
  ...
  user_choice=$(printf "Play from Online Stations\nPlay from Music Folder\nShuffle Play from Music Folder" | rofi -dmenu -config ~/.config/rofi/custom-rofi/config-rofi-Beats-menu.rasi -p "Select music source")
